...
func (r *userRepository) FindOne(db *gorm.DB, cnd *sqls.Cnd) *models.User {
	ret := &models.User{}
	if err := cnd.FindOne(db, &ret).Error; err != nil {
		return nil
	}
	return ret
}
...
func (r *userRepository) UpdateColumns(db *gorm.DB, id int64, name string, value interface{}) (err error) {
	err = db.Model(&models.User{}).Where("id = ?", id).UpdateColumn(name, value).Error
	return
}

func (r *userRepository) Delete(db *gorm.DB, id int64) (err error) {
	err = db.Delete("id = ?", id).Error
	return
}
