...
var ArticleRepository = newAraticleRepository()

func newAraticleRepository() *articleRepository {
...
func (r *articleRepository) Update(db *gorm.DB, id int64, columns map[string]interface{}) (err error) {
	err = db.Model(&models.Article{}).Where("id = ?", id).Updates(columns).Error
	return
}

func (r *articleRepository) Updates(db *gorm.DB, id int64, name string, value interface{}) (err error) {
